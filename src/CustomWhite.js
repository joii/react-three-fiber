...
export default function CustomWhite() {
  ...
  function Shoe() {
    ...
    useFrame((state) => {
     ...
     ref.current.rotation.set(Math.cos(t / 4) / 8, Math.sin(t / 4) / 8, 0.05 - (1 + Math.sin(t / 8)/4) / 20)
      ref.current.position.y = 1;//(1 + Math.sin(t / 1)) / 10
    })
